...
exports.deleteCountry=async(req,res)=>{
    constcountryId=req.params.id;
    try{
    // Verificar si el país existe
        const checkResult=awaitpool.query('SELECT * FROM country WHERE id = $1',[countryId]);
        if(checkResult.rows.length===0) {
        returnres.status(404).json({error:'País no encontrado'});
        }
        // Eliminar el país
